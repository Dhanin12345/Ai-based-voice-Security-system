...
class HumanDatasetAnalysis:
    """
    Dataset-Level Human Voice Analytics Service.
    Reads HUMAN class records from dataset/features.csv and computes statistical averages and feature distributions.
    """
    ...
    def get_analytics(self) -> Dict[str, Any]:
        """
        Computes summary statistics for all HUMAN voice samples.
        """
        ...
        human_df = df[df["class"].str.upper() == "HUMAN"]
        if human_df.empty:
            return self._empty_summary()
        ...
        def get_mean(col):
            return float(np.mean(human_df[col])) if col in human_df.columns else 0.0

        def get_std(col):
            return float(np.std(human_df[col])) if col in human_df.columns else 0.0
...
human_dataset_analysis = HumanDatasetAnalysis()
